...
static bool IsSlash(const QString& pszStart, size_t nPos);
static bool GetDirName(const QString& sDir, QString& sName);
...
PATH_EXISTENCE DoesPathExist(const QString& szPath, bool (*IsArchiveFile)(const QString&) /*= nullptr*/)
{
	...
	DWORD attr = GetFileAttributes(TFile(QString(lpcszPath)).wpath().c_str());
...
{
	return QFileInfo(path).completeSuffix(); // ::PathFindExtension(path.c_str());
}
...
QString GetLongPath(const QString& szPath, bool bExpandEnvs)
{
	...
	QString tPath = TFile(String(lpcszPath)).wpath();
...
PATH_EXISTENCE GetPairComparability(const PathContext & paths, bool (*IsArchiveFile)(const QString&) /*= nullptr*/)
{
	...
	if (paths.GetSize() < 2 || paths[0].isEmpty() || paths[1].isEmpty())
		return DOES_NOT_EXIST;
...
QString ConcatPath(const QString & path, const QString & subpath)
{
	...
	{
		return QString(path).append(subpath + (IsSlash(subpath, 0) ? 1 : 0));
	}
	else
	{
 ...
 */
void SplitFilename(const QString& pathLeft, QString* pPath, QString* pFile, QString* pExt)
{
	QString dirname = QDir(pathLeft).dirName();
	pPath = &dirname; // &QDir(pathLeft).dirName();
	QString filename = QFileInfo(pathLeft).baseName();
	pFile = &filename; //QFileInfo(pathLeft).baseName();
	QString extension = QFileInfo(pathLeft).completeSuffix();
	pExt = &extension; //QFileInfo(pathLeft).completeSuffix();
